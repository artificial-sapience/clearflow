...
1. **Purpose**: What role does this play in the protocol?
...
```lean
/-- Consensus mechanism for protocol decisions.
...
```lean
/-- The flourishing delta function measures change in collective wellbeing.
    
    Mathematically: ΔF(S, t₁, t₂) = ∑_{e ∈ S} w(e) · (f(e, t₂) - f(e, t₁))
    
    In English: The flourishing delta for a set S between times t₁ and t₂
    equals the sum over all entities e in S of the weight of e multiplied
    by the difference between e's flourishing at t₂ and t₁.
    
    Where:
    - S is a finite set of entities
    - w(e) is the positive weight assigned to entity e  
    - f(e,t) is entity e's flourishing level at time t
    - All summations converge due to S being finite
-/
noncomputable def flourishingDelta (S : Finset Entity) (t₁ t₂ : Time) : ℝ :=
  S.sum fun e => weight e * (flourishingAt e t₂ - flourishingAt e t₁)
...
- [Protocol] Section X.Y of protocol specification
-/
```
...
```lean
/-!
# Flourishing.Network.Consensus

Distributed consensus mechanisms for protocol governance.

## Overview

This module implements the consensus layer of the Flourishing Protocol,
enabling distributed decision-making without central authority. It builds
on the entity model from `Flourishing.Core` and uses the temporal logic
from `Flourishing.MathematicalFoundations`.
...
- [Protocol] Section 4.2 - Consensus Mechanisms
-/
```

## 5.4 Normative Choice Documentation

**Requirement**: Every value choice that affects protocol behavior MUST document its normative basis and revision mechanism.
...
```lean
import Flourishing.Meta.Decision
...
protocol objects that can be versioned, tracked, and evolved.
-/

namespace Flourishing.Core

open Flourishing.Meta
...
def DC_KNOW_1 : NormativeDecision := {
  ...
  rationale := "In Constructor Theory, knowledge is defined by what transformations it enables. Making this explicit ensures knowledge actively participates in the protocol rather than being passive data."
  dependencies := []
  status := .Active
