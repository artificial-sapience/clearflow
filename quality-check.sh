...
if [ -d "clearflow" ]; then
    if ! uv run xenon --max-average A --max-modules A --max-absolute A clearflow/ 2>&1; then
    ...
    echo -e "${YELLOW}Skipping Xenon (no clearflow directory)${NC}"
fi
...
if [ -d "clearflow" ]; then
    ...
    echo -e "${YELLOW}Skipping Vulture (no clearflow directory)${NC}"
fi
...
if [ -d "clearflow" ]; then
    avg_cc=$(uv run radon cc clearflow/ -a -j | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('clearflow', {}).get('average_complexity', 0))" 2>/dev/null || echo "0")
    ...
    echo -e "${YELLOW}Skipping Radon (no clearflow directory)${NC}"
fi
